...
docker build -t gcr.io/YOUR_PROJECT_ID/l0169:latest .

# Push to Container Registry
docker push gcr.io/YOUR_PROJECT_ID/l0169:latest

# Deploy to Cloud Run
gcloud run deploy l0169 \
  --image gcr.io/YOUR_PROJECT_ID/l0169:latest \
  --platform managed \
  --region us-central1 \
  --port 50169 \
...
- `PORT`: Automatically set by Cloud Run (the app defaults to 50169)
...
```bash
gcloud run services update l0169 \
  --region us-central1 \
...
gcloud run services update-traffic l0169 \
  --region us-central1 \
  --to-latest
...
1. **Port Configuration**: Ensure the app uses `process.env.PORT` and falls back to 50169
2. **Memory Limits**: Cloud Run defaults to 256MB. Increase if needed:
   ```bash
   gcloud run services update l0169 --memory 512Mi --region us-central1
   ```
3. **Cold Starts**: Cloud Run may have cold starts. Consider minimum instances:
   ```bash
   gcloud run services update l0169 --min-instances 1 --region us-central1
   ```
...
```bash
# List revisions
gcloud run revisions list --service l0169 --region us-central1
